...
def translate_coordinates(x, y, app_name):
    ...
    # Default translation for native apps
    translated_x = int(x / scaling_factor)
    translated_y = int(y / scaling_factor)
    
    # Additional scaling for Chrome/browser windows
    if "chrome" in app_name.lower() or "firefox" in app_name.lower() or "safari" in app_name.lower():
        # This factor might need adjustment based on testing
        browser_scaling_factor = 1.5  
        translated_x = int(translated_x * browser_scaling_factor)
        translated_y = int(translated_y * browser_scaling_factor)
...
def calibrate():
    ...
    print("1. Make sure the window you want to calibrate for is active.")
    print("2. Move your mouse to the top-left corner of the content area.")
    input("Press Enter when ready...")
    top_left = pyautogui.position()
    print(f"Top-left position: {top_left}")
    
    print("Now move your mouse to the bottom-right corner of the content area.")
    input("Press Enter when ready...")
    bottom_right = pyautogui.position()
    print(f"Bottom-right position: {bottom_right}")
    
    app_name = get_active_window_info()
    print(f"Calibration for: {app_name}")
    print(f"Content area: from {top_left} to {bottom_right}")
    
    print("You can use these values to adjust the translate_coordinates function for this application.")
...
if __name__ == "__main__":
    ...
    print("Advanced Mac Image Detection Script")
    print("------------------------------------")
