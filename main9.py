...
def translate_coordinates(x, y, app_name):
    ...
    if "chrome" in app_name.lower() or "firefox" in app_name.lower() or "safari" in app_name.lower():
        if window_bounds:
            ...
            translated_x = int((x - window_bounds[0]) / scaling_factor) + window_bounds[0]
            translated_y = int((y - window_bounds[1]) / scaling_factor) + window_bounds[1]
        else:
...
if __name__ == "__main__":
    ...
    print("Enhanced Debug Mac Image Detection Script")
    print("------------------------------------------")
