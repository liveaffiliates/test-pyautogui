...
if __name__ == "__main__":
    ...
    parser.add_argument("image_path", help="Path to the image file to detect")
    ...
    print("Flexible Mac Image Detection Script")
    print("------------------------------------")
    
    if args.click:
        success = click_image(args.image_path, confidence=args.confidence, debug=args.debug)
        print(f"Click operation {'successful' if success else 'failed'}.")
    else:
        result = detect_image(args.image_path, confidence=args.confidence, debug=args.debug)
        if result:
